...
export const SearchPanelMin = (props) => {
    ...
    return (
        <div className="search-panel-min">
            <button onClick={props.showBigSearchPanel} className="search-panel-min__place">{`${filtersData.city}, ${filtersData.country}`}</button>
            <button onClick={props.showBigSearchPanel} className="search-panel-min__guest">{filtersData.numOfGuest <= 0 ? 'Add guests' : `${filtersData.numOfGuest} guests`}</button>
